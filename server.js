...
// API to fetch original data from fuel_consumption_ratings table
app.get('/api/fuel_consumption_ratings', async (req, res) => {
  try {
    const data = await fetchData('SELECT * FROM fuel_consumption_ratings');
    res.json(data);
  } catch (err) {
    console.error('Error in /api/fuel_consumption_ratings:', err.message);
    res.status(500).send('Error retrieving data from database');
  }
});
